...
/* C O N T E N T S
 *
 * A MEX gateway routine for f_apd_demodulation:
 *
 * (1) mexFunction.
 */
...
/* P U R P O S E
 *
 * f_apd_demodulation_mex estimates the amplitude modulator of a given signal in the
...
/***********************************************************************************/
    ...
    /* Checking if the 1st input argument is a real array with right dimensions */
    ...
    if ( !mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]) || N < 2 || D > 3 )
    ...
    else if ( mxGetNumberOfElements(pr_in2) != D || !mxIsDouble(pr_in2) || \
              mxIsComplex(pr_in2) )
        
        mexErrMsgIdAndTxt("AP_Demodulation:InpVal", "Field 'Fs' of the 2nd input "\
                          "argument must be a real array with the number of "\
    ...
    else if ( !mxIsScalar(pr_in2) || !mxIsDouble(pr_in2) || mxIsComplex(pr_in2) )
    ...
    else if ( mxGetNumberOfElements(pr_in2) != 1 || !mxIsDouble(pr_in2) || \
              mxIsComplex(pr_in2) )
    ...
    if ( nrhs == 4 && !mxIsEmpty(prhs[3]) && pr_in2 == NULL )
        ...
        mexErrMsgIdAndTxt("AP_Demodulation:InpVal", "The 2nd input argument must "\
            ...
            (mxGetNumberOfElements(pr_in2) != D || !mxIsDouble(pr_in2) || \
            mxIsComplex(pr_in2)) )
        
        mexErrMsgIdAndTxt("AP_Demodulation:InpVal", "Field 'Nr' of the 2nd input "\
                          "argument must be a real numeric array with the number of"\
                          " elements equal to the dimension of the signal!");
    ...
    if ( pr_in2 == NULL || mxIsEmpty(pr_in2) )
    {
        ...
        if (Par.im == NULL)
        {
            f_apd_set_error(APD_ERR_ID_MEM,__LINE__,APD_ERR_FILE); goto finish;}
    ...
    if ( nrhs > 2 && !mxIsEmpty(prhs[2]) )
    {
        ...
        if ( !mxIsDouble(prhs[2]) || mxIsComplex(prhs[2]) || n != N )
            
            mexErrMsgIdAndTxt("AP_Demodulation:InpVal", "The third input argument "\
                              "must be either empty or a real array with the same "\
                              "number of elements as the input signal!");
    ...
    if ( nrhs > 3 && !mxIsEmpty(prhs[3]) )
    {
        ...
        if ( !mxIsDouble(prhs[3]) || mxIsComplex(prhs[3]) || \
             mxGetNumberOfDimensions(prhs[3]) != 2 || sz_t[0] != N || sz_t[1] != D )

            mexErrMsgIdAndTxt("AP_Demodulation:InpVal", "The 4th input argument "\
                              "must be either empty or a real 2D array with the "\
                              "number of columns equal to the dimension of the "\
                              "input signal and the number of rows equal to the "\
...
/***********************************************************************************/
    ...
    exitflag = f_apd_demodulation (pr_s, &Par, Ub, pr_t, pr_m, pr_e, &iter);
...
/***********************************************************************************/
    ...
    finish:
        ...
        if (exitflag != APD_ERR_ID_NON)
        {
            f_apd_print_error (exitflag);

            mexErrMsgIdAndTxt("AP_Demodulation:IV", "");
        }
